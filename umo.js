class Umo { //Universal Moving Object
	constructor(xxx, yyy, sss, ccc) {
		...
		this.hp = 400 ; //This is used for destructible entities to track health
		this.maxhp = 400;
	...
	drawship(viewx, viewy){ //Ships are drawn as polar polygons, a triangle is the default.  Viewx/viewy are camera center
		...
		var shieldthick = Math.floor(this.shield*2/this.maxshield); //shield
		...
		context.fillRect(x-this.s/2, y+this.s, Math.floor(this.s*prop), 2);
		}
	drawplanet(viewx, viewy){ //input variables are player ship/camera position
		...
		context.lineWidth = 4; //circle is thicc
		context.stroke();	//ok now actually draw it.
		if ((this.c2!==0)&&(this.s>16)){
			context.beginPath();
			context.strokeStyle = this.c2; //sets planet color
			context.arc(x, y, this.s-2, 0, 2 * Math.PI, false); //draws the 2nd outermost circle
			context.lineWidth = 4; //circle is thicc
			context.stroke();	//ok now actually draw it.
			context.beginPath();
			context.fillStyle = this.c; //sets planet color
			context.arc(x, y, this.s-4, 0, 2 * Math.PI, false); //draws the innermost circle
			context.lineWidth = 4; //circle is thicc
			...
			while(i<this.polytheta.length){
				...
				var blobdistance = this.s - blobsize-2;
	...
	updateship(theplanets){//Unfortunately I need access to the list of planets to handle ship respawning.
		...
		this.y = this.y + this.vy;
		this.d = this.d + this.vd;
		if ((this.hp <= 0) && (this.hp !==-1000)){ this.killship(600); }
	...
	updatebomb(){ //Handles timer and explosions
		...
		if (this.timer == 1){this.s = 50*this.boombuff;}
		if (this.timer == 2){this.s = 64*this.boombuff;}
		if (this.timer == 3){this.s = 60*this.boombuff;}
		if (this.timer == 4){this.s = 50*this.boombuff;}
		if (this.timer == 5){this.s = 38*this.boombuff;}
		if (this.timer == 6){this.s = 25*this.boombuff;}
		if (this.timer>6){
			this.s = 4; //Mostly redundant
	...
	setorbit(parentplanet, distance, direction, cw){ //cw = -1 or 1
		...
		var gravy = parentplanet.m*.003 / (distance*distance);  //gMm/r^2, where m is 1;
